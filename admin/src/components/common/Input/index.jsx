...
const Input = ({ labelTitle, placeholder, attribute, register, errors, textArea = false }) => {
    ...
    return (
        <div className='flex gap-4 flex-col'>
            <label htmlFor={'input'+uuid} className='text-2xl font-bold text-Pgreen w-fit'>{labelTitle}:</label>
            {
                textArea
                ?
                <textarea {...register} type={(['confirmPassword', 'password'].includes(attribute)) ? 'password' : "text"} id={'input'+uuid} className='resize-none rounded-xl border-2 border-Pgreen px-4 pt-2 pb-3 shadow-lg transition-all hover:shadow-none outline-none' placeholder={placeholder} />
                :
